function InvitesView({ invites, onAcceptInvite }) {
  return (
    <section className="rounded-2xl border border-blue-900/60 bg-slate-900/70 p-6">
      <h2 className="text-2xl font-semibold text-white">Team Invites</h2>
      <p className="mt-1 text-sm text-blue-100/80">Accept an invite to join a team ranking.</p>

      {!invites.length ? (
        <p className="mt-4 text-sm text-blue-100/80">No pending invites.</p>
      ) : (
            ...
            >
              <div>
                <p className="text-white">
                  <span className="font-semibold">{invite.teamName}</span> invited by @{invite.inviterUsername}
                </p>
                <p className="text-xs text-blue-100/70">Pending invite</p>
              </div>
              <button
                type="button"
